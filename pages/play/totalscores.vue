<!-- Player Total Scores - Final results -->
<template>
  <section class="play-page play-quiz">
    <v-container fluid>
      <div class="play-page-container">
        ...
        <!-- My result highlight -->
        <div v-if="myResult" class="my-result-card text-center pa-6 mb-4">
          <v-icon size="60" :color="myResult.rank === 1 ? 'amber' : '#ff5e94'">
            {{ myResult.rank <= 3 ? 'mdi-trophy' : 'mdi-account-star' }}
          </v-icon>
          <h2 class="white--text mt-2">{{ myResult.name }}</h2>
          ...
          <div class="table-content w-100 mx-auto">
            ...
            >
              <div class="table-body-content d-flex align-center justify-center w-100"
                :class="{ 'my-row': entry.name === playerName }">
